...
function _isModel(obj, sequelize, seqVer) {
  if (seqVer >= 5) return obj.prototype instanceof Model
  if (seqVer >= 4) return obj.prototype instanceof sequelize.Model
  return obj instanceof sequelize.Model
}

function _isModelInstance(obj, sequelize, seqVer) {
  if (seqVer >= 5) return obj instanceof Model
  if (seqVer >= 4) return obj instanceof sequelize.Model
  return obj instanceof sequelize.Instance
}

function _getModelFromInstance(inst, seqVer) {
  return seqVer >= 4 ? inst.constructor : inst.Model
}

function _isSpecificModelInstance(obj, model, seqVer) {
  return seqVer >= 4 ? (obj instanceof model) : (obj instanceof model.Instance)
}
...
class Serializer {
  constructor(model, scheme, options) {
    ...
    if (!(sequelize.Model || Model) || isNaN(seqVer) || !_isModel(model, sequelize, seqVer)) {
  ...
  _serializeValue(attr, value, cache) {
    ...
    if (_isModelInstance(value, this._seq, this._seqVer)) {
